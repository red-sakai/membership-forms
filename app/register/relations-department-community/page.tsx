...
const communityQuestionFieldNames = [
  "memberEngagementQuestion1",
  "memberEngagementQuestion2",
  "memberEngagementQuestion3",
  "memberRelationsSupportQuestion1",
  "memberRelationsSupportQuestion2",
  "memberRelationsSupportQuestion3",
] as const;
...
export default function RelationsDepartmentCommunityPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    communityQuestionFieldNames.forEach((fieldName) => {
    ...
    const { error } = await supabase.from("registration_relations_community_department").insert({
    ...
    const { error: interviewError } = await supabase.from("to_be_interviewed").insert({
      name: fullName,
      email,
      department: "Relations Community",
      team: selectedTeam,
      role: selectedRole,
      status: "pending",
    });
