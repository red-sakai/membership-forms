...
export default function OSITLeadPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const expectationAnswer = String(
      formData.get(position === "lead" ? "osItLeadExpectation" : "osItCoLeadExpectation") ?? "",
    );
    const certificationsAnswer = String(
      formData.get(position === "lead" ? "osItLeadCertifications" : "osItCoLeadCertifications") ?? "",
    );
    ...
    setIsSubmitting(false);
    router.push("/register/technology-department/os-it-lead/submit");
  };
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-sky-50 via-blue-50 to-indigo-100 px-4 py-6 font-sans text-zinc-900">
      <main className="mx-auto w-full max-w-3xl rounded-2xl border border-sky-100 bg-white/95 p-6 shadow-lg shadow-blue-100 sm:p-8">
        <h1 className="text-2xl font-semibold text-slate-900 sm:text-3xl">Registration - OS &amp; IT Lead / Co-Lead</h1>
        <p className="mt-4 text-sm leading-6 text-slate-700">
          The OS &amp; IT Department  is responsible for strengthening members’ technical foundation in computer hardware, operating systems, and IT infrastructure. This role involves mentoring cadets in system configuration, troubleshooting, and Linux administration while ensuring that organizational systems remain efficient, secure, and reliable.
        </p>
        ...
        >
          <fieldset className="space-y-3 rounded-xl border border-sky-200 bg-sky-50/70 p-4">
            ...
            <div className="space-y-2 text-sm text-slate-700">
              ...
              <label className="flex items-start gap-3 text-sm">
                <input
                  ...
                  required
                />
                <span>OS &amp; IT Co-Lead</span>
              </label>
            </div>

            {position === "lead" && (
              <section className="rounded-lg border border-sky-200 bg-white p-4 text-sm text-slate-700">
                <h2 className="font-semibold text-slate-900">OS &amp; IT Lead Responsibilities:</h2>
                <ul className="mt-2 list-disc space-y-1 pl-5">
                  <li>Mentor members in computer hardware setup, operating system installation, and IT maintenance.</li>
                  <li>Oversee system configuration, updates, and troubleshooting processes.</li>
                  <li>Lead initiatives in system optimization, security, and documentation.</li>
                </ul>

                <label className="block space-y-2">
                  <span className="font-medium">
                    How do you expect to lead the OS &amp; IT Department if we were to choose you as a lead? <span className="text-red-600">*</span>
                  </span>
                  <textarea
                    name="osItLeadExpectation"
            ...
            {position === "co-lead" && (
              <section className="rounded-lg border border-sky-200 bg-white p-4 text-sm text-slate-700">
                <h2 className="font-semibold text-slate-900">OS &amp; IT Co-Lead Responsibilities:</h2>
                <ul className="mt-2 list-disc space-y-1 pl-5">
                  <li>Support the Lead in mentoring members on hardware setup, OS installation, and IT maintenance.</li>
                  <li>Assist in managing system configuration, updates, and troubleshooting workflows.</li>
                  <li>Help drive initiatives for system optimization, security improvements, and documentation.</li>
                </ul>

                <label className="block space-y-2">
                  <span className="font-medium">
                    How do you expect to lead the OS &amp; IT Department if we were to choose you as a co-lead? <span className="text-red-600">*</span>
                  </span>
                  <textarea
                    name="osItCoLeadExpectation"
                    className="min-h-24 w-full rounded-md border border-slate-300 bg-white px-3 py-2 outline-none focus:border-sky-500"
