...
export default function NetworkingLeadPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const expectationAnswer = String(
      formData.get(position === "lead" ? "networkingLeadExpectation" : "networkingCoLeadExpectation") ?? "",
    );
    const certificationsAnswer = String(
      formData.get(position === "lead" ? "networkingLeadCertifications" : "networkingCoLeadCertifications") ?? "",
    );
    ...
    const { error } = await supabase.from("registration_technology_lead_colead").insert({
      ...
      last_name: lastName,
      email,
      technology_department: "Networking",
